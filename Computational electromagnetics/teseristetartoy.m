...
S = C*diag(ones(M*M,1))+ry*diag(ones(M*M-1,1),1)+ry*diag(ones(M*M-1,1),-1)+rx*diag(ones(M*M-50,1),50)+rx*diag(ones(M*M-50,1),-50);
B = D*diag(ones(M*M,1))-ry*diag(ones(M*M-1,1),1)-ry*diag(ones(M*M-1,1),-1)-rx*diag(ones(M*M-50,1),50)-rx*diag(ones(M*M-50,1),-50);
Ss = sparse(S);
Bs = sparse(B);
...
%}
selected_values = reshape(selected_values, 50, 50);
