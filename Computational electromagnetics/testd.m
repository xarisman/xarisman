...
Lx = 10*10^-6; Ly = Lx;
hx = 10^-6; wx = 10^-6;
N = 40; M=N; dx = Lx/N; dy = Ly/M; dz = 0.4*10^-6; 
lamda = 1.550*10^-6; 
k_0 = 2*pi/lamda; 
R = 5*10^-6; 
a = 0.5; w_0 = 1*10^-6;
...
%  for ii=1:N
%      for jj=1:N
%         double_sum_12 = double_sum_12 + log(E_gamma_dz(ii, jj)/E_gamma(ii,jj))*(abs(E_gamma(ii,jj))^2);
%      end
% end
% for ii=1:N
%      for jj=1:N
...
for ii=1:N
     for jj=1:N
         %AA = diff(E_0, 2);
         A = abs(diff(E_gamma, N+38))/N/N;
        double_sum_13 = double_sum_13  + (k_0^2*n_i_j(ii, jj)^2-k_ref^2)*abs(E_gamma(ii, jj))^2-A^2;
     end
end
for ii=1:N
     for jj=1:N
        double_sum_23 = double_sum_23 + 2*k_ref*abs(E_gamma(ii, jj))^2;
     end
end
