...
for i = 1:length(SNR)
    ...
    s = ip/sqrt(1/(2*p));
    n = 1/sqrt(16*p)*(randn(1,N) + (1j)*randn(1,N));
    ...
    s2 = ip2./sqrt(1/(2*(1-p)));
    n2 = 1/sqrt(32*(1-p))*(randn(1,N) + (1j)*randn(1,N));
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %Theoretical results
    Ser2(i) = 1/2*erfc(sqrt((sind(th).^2).*4*(1-p)*(10.^(i/10))))+1/4.*erfc(sqrt((cosd(th)-sind(th)).^2.*4*(1-p)*(10.^(i/10))));
    Ser1(i) = 1/2*erfc(sqrt((sind(th).^2).*2*p*(10.^(i/10))))+1/4.*erfc(sqrt((cosd(th)-sind(th)).^2.*2*p*(10.^(i/10))));
end
